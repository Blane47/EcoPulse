...
const buildMapHTML = (bins, isDark) => {
  const geojson = {
  ...
  const popupTextColor = isDark ? '#f3f4f6' : '#1f2937';
  const popupSubColor = isDark ? '#9ca3af' : '#6b7280';
  const popupBg = isDark ? '#222536' : '#ffffff';
        ...
        map.on('load', () => {
          ...
          map.addLayer({
            ...
            paint: {
              ...
              'circle-stroke-color': '${isDark ? '#222536' : '#ffffff'}',
              'circle-opacity': 0.9,
            },
          });
          ...
          map.addLayer({
            ...
            paint: {
              ...
              'text-halo-color': '${isDark ? '#1a1d29' : '#ffffff'}',
              'text-halo-width': 1.5,
            },
          });
...
export default function NearbyMapScreen() {
  ...
  if (loading) return <View style={[styles.loading, { backgroundColor: c.background }]}><ActivityIndicator size="large" color={c.accent} /></View>;

  return (
    <View style={styles.container}>
      {/* Legend overlay */}
      <View style={[styles.legendOverlay, { backgroundColor: darkMode ? 'rgba(34,37,54,0.92)' : 'rgba(255,255,255,0.92)' }]}>
      ...
      <WebView
        style={styles.map}
        source={{ html: buildMapHTML(bins, darkMode) }}
        javaScriptEnabled
        originWhitelist={['*']}
        scrollEnabled={false}
      />
